...
def which_postcode(postcode: str) -> str:
    pattern_GB = r'^[A-Za-z]{1,2}\d{1,2} \d[A-Za-z]{2}$'
    pattern_SK = r'^\d{3} \d{2}\b$'
    postcode = postcode.strip()
    return 'GB' if re.findall(pattern_GB, postcode) else 'SK' if re.findall(pattern_SK, postcode) else 'Not valid'