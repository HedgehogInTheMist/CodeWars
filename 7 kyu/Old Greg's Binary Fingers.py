...
def binary_fingers(bin_string: str) -> list:
    fingers = ['Pinkie', 'Ring', 'Middle', 'Index', 'Thumb']
    bin_string = f'{bin_string:0>5}'
    return [i for i, j in zip(fingers, bin_string) if j == '1']


def binary_fingers(bin_string: str) -> list:
    fingers = ['Pinkie', 'Ring', 'Middle', 'Index', 'Thumb']    
    return [i for i, j in zip(fingers, f'{bin_string:0>5}') if j == '1']
