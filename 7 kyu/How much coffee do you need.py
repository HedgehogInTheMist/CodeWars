...
def how_much_coffee(events: list) -> int:
    return 'You need extra sleep' if (result := sum(2 if i.isupper() else 1 for i in events if i.lower() in ['cat', 'dog', 'cw', 'movie'])) > 3 else result